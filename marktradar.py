...
def mache_prompt(defekt, beschreibung, url_text):
    ...
    return (
        ...
        f"ZUSTAND-SELBSTEINSCHÄTZUNG (aus dem Bild):\n"
        f"- Erkannter Zustand: [sehr gut / gut / gebraucht / beschädigt]\n"
        f"- Sichtbare Mängel: [alle Kratzer, Dellen, Chips einzeln nennen oder 'keine erkennbar']\n"
        f"- Defektgrad (eigene Einschätzung): X%\n\n"

        f"PREISE (jeweils NUR eine konkrete Zahl — keine Spannen!):\n"
        f"Aktueller Zustand ({zustand}, {defekt}% Defekt):\n"
        f"- eBay: €[Zahl]\n"
        f"- Kleinanzeigen: €[Zahl]\n"
        f"- Vinted: €[Zahl]\n"
        f"- Facebook: €[Zahl]\n"
        f"- Flohmarkt: €[Zahl]\n"
        f"- Max. Ankaufspreis jetzt: €[Zahl]\n\n"

        f"Minimaler Defekt (aufbereitet / gereinigt / wie neu):\n"
        f"- eBay optimal: €[Zahl] (+€[Differenz] durch Aufbereitung)\n"
        f"- Kleinanzeigen optimal: €[Zahl]\n"
        f"- Flohmarkt optimal: €[Zahl]\n"
        f"- Max. Ankaufspreis bei perfektem Zustand: €[Zahl]\n\n"

        f"Wertsteigerungs-Potenzial:\n"
        f"- Jetzt verkaufen: €[Zahl]\n"
        f"- Nach Aufbereitung: €[Zahl]\n"
        f"- Mehrwert durch Aufbereitung: +€[Zahl] (+X%)\n\n"
